...
function initApp(b6, el) {
        ...
        fn(username, pass, function(err, token) {
            if (err) {
                ...
                b6.session.external(token, function(err) {
                    ...
                    if (err) {
                        $('#authError').html('<p>Bit6 auth failed</p>');
                    }
                    else {
                        $('#authUsername').val('');
                        $('#authPassword').val('');
    ...
    function signup(username, password, cb) {
        el.Users.register(username, password, {},
            function(data) {
                console.log('Signed up', data.result);
    ...
    // Telerik auth complete.
    // Get Bit6 user auth token from the cloud function
    function getBit6AuthToken(cb) {
